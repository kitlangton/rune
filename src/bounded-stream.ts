...
import { capabilityError, type CapabilityError } from "./capability-error.js"

export const readBoundedText = <E, R>(
  stream: Stream.Stream<Uint8Array, E, R>,
  maxBytes: number,
  label: string,
): Effect.Effect<string, E | CapabilityError, R> => stream.pipe(
  Stream.runFoldEffect(
    () => ({ chunks: [] as Array<Uint8Array>, size: 0 }),
    (collected, chunk) => {
      const size = collected.size + chunk.byteLength
      if (size > maxBytes) return Effect.fail(capabilityError(`${label} exceeds ${maxBytes} bytes.`))
